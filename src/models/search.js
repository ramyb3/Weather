...
function Search()
{
    ...
    const check= async ()=> // search func
    {
        if(search.search(regex)!=-1) //check english letters
        alert('Please write in English!');

        else
        {
            if(search!='') //if there's a search
            {
                try
                {
    ...
    return(<div style={{textAlign: 'center'}}> 
        ...
        <button onClick={check} style={{cursor: 'pointer'}}><FontAwesomeIcon icon={faSearch}/></button><br/><br/>

        {cities.length!=0 ? //if there's a search
            <div className='searchResults'>
                {
                    cities.map((x,index)=> // shows city and country names
                    {
                        return <li className='result' key={index} onClick={()=>result([x.LocalizedName,x.Key])}>{x.LocalizedName}, {x.Country.LocalizedName}</li>
                    })
                }
            </div> : null
        }

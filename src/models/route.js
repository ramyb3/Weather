...
function RouteComp()
{
    ...
    return(<>
        <Routes>
            <Route path="/" element={<HomePage favorites={props} callback={data => setProps(data)}/>}/>
            <Route path="favorites" element={<Favorites favorites={props}/>}/>
        </Routes>
    </>)
}

export const FtoC= (f)=> // Fahrenheit to Celsius 
{
    let c= (f-32.00)/1.8000;

    return c.toFixed(1);
}

export const day= (date)=> // get day 
{
...
export const getData= async(obj,method)=> // get data from api
{
    ...
    if(method==1)
    {
        let resp= await axios.get(`https://dataservice.accuweather.com/currentconditions/v1/${obj}?apikey=${key}`);
        return resp.data[0]; //get now city weather
    }

    if(method==2)
    {
        let resp= await axios.get(`https://dataservice.accuweather.com/forecasts/v1/daily/5day/${obj}?apikey=${key}`);
    ...
    if(method==4)
    {
        let resp= await axios.get(`https://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey=${key}&q=${obj[0]}%2C${obj[1]}`);

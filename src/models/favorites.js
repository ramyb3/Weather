...
function Favorites(props)
{
    ...
    const display= async(x)=> //show clicked favorite in home page
    {
        try
        {
            ...
            dispatch({type: "LOAD", payload: [weather[x],days,[props.favorites.name[x],props.favorites.key[x]]]});

            // home button page will be colored
            document.getElementById('fav').style='';
            document.getElementById('home').style.backgroundColor= 'rgb(123, 185, 171)';
        ...
        <div className="container" style={{flexWrap: 'wrap'}}>
            {
                ...
                {
                    return <div key={index} className="favorites" onClick={()=>display(index)}>
                        
                        {props.favorites.name[index]}<br/> {/*location name*/}

...
function Home(props)
{
    ...
    const fav=(e)=> // favorites func
    {
        if(!props.favorites.key.includes(storeData[0][2][1]))
        {
            props.callback({key: [...props.favorites.key, storeData[0][2][1]], 
        ...
        else
        {
            ...
            props.callback({key: props.favorites.key.filter(x=> x!=storeData[0][2][1]), 
        ...
        {storeData.length!=0 ? // if the store finished to load all data
            <div className='box'>
                ...
                {storeData[0][0].WeatherIcon >9 ? // weather icon
                    <img src={`https://developer.accuweather.com/sites/default/files/${storeData[0][0].WeatherIcon}-s.png`}/>
                    :
                    <img src={`https://developer.accuweather.com/sites/default/files/0${storeData[0][0].WeatherIcon}-s.png`}/>
                }   

                <div className='all'>{storeData[0][2][0]}</div> {/*city's name */}

                {storeData[1]==true ? // temp in C/F
                    <div className='all'>{storeData[0][0].Temperature.Imperial.Value} &#778; F</div>
                    : 
                    <div className='all'>{storeData[0][0].Temperature.Metric.Value} &#778; C</div>
                }

                {props.favorites.key.includes(storeData[0][2][1]) ? // this city favorite or no
                    <FontAwesomeIcon style={{color: 'red'}} icon={faHeart} onClick={(e)=>fav(e.target)} className='heart'/> 
                    : 
                    <FontAwesomeIcon icon={faHeart} onClick={(e)=>fav(e.target)} className='heart'/>
                } 

                <div className='weather'>{storeData[0][0].WeatherText}</div> {/*current weather*/}

                <div className='container' style={{flexWrap: 'wrap'}}>
                    {
                        storeData[0][1].map((x,index)=> //5 days weather forecast
                        {
